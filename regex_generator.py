...
from regexfactory.patterns import (
    ...
    Group,
    Or,
    # Work on character level (need to wrap input into Group in order to
    # become string-level)
    Amount,
    Multi,
    Optional
...
class DynamicCharGenerator:
    ...
    def __init__(self, set_complexity: int, amount_complexity: int):
        ...
        self._start_candidates.extend(
            DynamicCharGenerator.special_chars_without_any)
    ...
    def get_random_chars(self, length: int) -> typing.List[RegexPattern]:
        ...
        candidates.append(
            DynamicWrapper.wrap_into_amount(
    ...
    def _get_random_non_repeating_chars(
        ...
        """
        candidates = DynamicCharGenerator.special_chars_without_any + \
            DynamicCharGenerator.printable_escapes
        try:
...
class DynamicGroupGenerator:
    ...
    def __init__(self, set_complexity: int, union_complexity: int, amount_complexity: int,
        ...
        self._dynamic_char_generator = DynamicCharGenerator(
    ...
    def _get_random_group_pattern(self, recurse: int = 0) -> Group:
        ...
        if recurse > self._depth_complexity:
            ...
            return Group(
                join(*self._dynamic_char_generator.get_random_chars(length)))
        else:
